...
class overtable(Node):
    ...
    def __init__(self):
        ...
        self.bcu_publisher = self.create_publisher(Float32, '/bcu', 10)
        ...
        self.mean_value = 0.0
        self.values = deque(maxlen=300) #calculate mean of last 3 seconds
        self.delta_height = 0.0
        self.depthnav_active = False
        self.depth_reached = False #this is sent by the STM32 when the depth is reached
    ...
    def distance_callback(self, msg):
        ...
        if len(self.values) == 300:  # Only compute mean when we have 100 values
            self.mean_value = np.mean(self.values)
            self.get_logger().info(f"Mean of last 100 values: {self.mean_value:.2f}")
        ...
        try:
            while rclpy.ok():
                ...
                if (
                    ...
                    self.get_logger().info('Target depth and position reached.')
                    goal_handle.succeed()
                ...
                if (
                    ...
                    bcu_msg = Float32()
                    bcu_msg.data = self.delta_height
