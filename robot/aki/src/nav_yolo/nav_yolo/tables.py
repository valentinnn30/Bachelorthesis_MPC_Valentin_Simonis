...
class MarkerPublisher(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('camera_height', 480) #y
        self.declare_parameter('camera_width', 640) #x
    ...
    def listener_callback(self, detections_msg):
        ...
        for detection in detections_msg.detections:
            if (detection.class_name == "Table" or detection.class_name == "table"):
                if detection.score > best_score:
                    best_detection = detection
                    best_score = detection.score

        # skip if no table is detected
        if best_detection is None:
            return
        
        # use best detection
        box_msg: BoundingBox2D = best_detection.bbox
        ...
        if not self.detected_table:
            #self.get_logger().info(f"First table detected at: x={x}, y={y}, z={self.last_z}")
            self.detected_table = True
            self.last_logged_x = x
            self.last_logged_y = y
            self.marked_table = True

        
        elif (self.last_logged_x is None or self.last_logged_y is None or 
            abs(self.last_logged_x - x) > self.log_threshold or abs(self.last_logged_y - y) > self.log_threshold):
            #self.get_logger().info(f"Table position: x={x}, y={y}, z={self.last_z}")
            self.last_logged_x = x
            self.last_logged_y = y
            self.marked_table = True
    ...
    def publish_marker(self, x, y, z):
        ...
        # if self.marked_table:
        #     self.get_logger().info(f"Published marker at: ({x}, {y}, {z})")
