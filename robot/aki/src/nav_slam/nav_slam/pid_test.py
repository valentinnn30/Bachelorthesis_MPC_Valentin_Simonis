...
class overtable(Node):
    ...
    def __init__(self):
        ...
        # PID parameters
        self.declare_parameter('kp_x', 0.35) #0.35
        self.declare_parameter('ki_x', 0.01) #0.2
        self.declare_parameter('kd_x', 0.15) #0.1
        ...
        # Depth logic
        self.declare_parameter('test_with_depth', False)
        self.test_with_depth = self.get_parameter('test_with_depth').get_parameter_value().bool_value
        self.depth = 0.0
        self.depthnav_active = False
        self.depth_reached = False
        self.slam_height = 0.7
        ...
        self.timer_write = self.create_timer(0.2, self.execute_callback) # 5 Hz
    ...
    def execute_callback(self):
        ...
        if not self.test_with_depth:
            # Simple SLAM test mode
            if (
                abs(self.target_x - self.x) < 0.04 and
                abs(self.target_y - self.y) < 0.04
            ):
                self.get_logger().info('Target position reached (no depth control).')
                #goal_handle.succeed()
                #return result
        ...
        """try:
            while rclpy.ok():
                ...
                if not self.test_with_depth:
                    ...
                    if (abs(self.depth - self.goal_depth) <= 0.01):#mit Toleranz von 0.01 m, 30cm über den Korallen
                    ...
                    if (
                        abs(self.target_x - self.x) < 0.05 and
                        abs(self.target_y - self.y) < 0.05 and 
                        self.depth_reached and not self.feeding_done
                        ...
                        self.bcu_publisher.publish(bcu_msg) #additionally min distance with distance sensor, if possible
                        ...
                        #feeding finished, now go back to SLAM-height
                        self.goal_depth = self.depth - self.delta_depth

                        #bcu_msg.data = self.goal_depth
                        bcu_msg.data = 0.0
                        #self.start_depth = self.depth #start depth after feeding
                        self.bcu_publisher.publish(bcu_msg)
                        self.depth_reached = False #not really necessary, since recalculation in next loop
                        self.feeding_done = True

                    # Constant depth during slam movement
                    if ((abs(self.target_x - self.x) > 0.05 or abs(self.target_y - self.y) > 0.05)
                        and not self.depthnav_active):
                        if self.counter == 0:
                            bcu_msg = Float32()
                            #bcu_msg.data = self.depth
                            bcu_msg.data = 0.0
                    ...
                    if abs(self.target_x - self.x) < 0.05 and abs(self.target_y - self.y) < 0.05:
                        if not self.depthnav_active:
                            self.get_logger().info('Target position reached, sending target depth to BCU...')
                            self.goal_depth = self.depth + self.delta_depth
                            bcu_msg = Float32()
                            bcu_msg.data = self.goal_depth
                            self.depth_publisher.publish(bcu_msg)
                        ...
                        # if abs(self.depth - self.goal_depth) <= 0.05:
                        #     self.get_logger().info('Depth reached, goal succeeded.')
                        #     goal_handle.succeed()
                        #     return result
    ...
    def timer_callback(self):
        # Calculate errors
            ...
            # Cap forces at [-1, 1]
            #force_x = max(-0.5, min(0.5, force_x)) # no need for this, would also destroy the proportions
