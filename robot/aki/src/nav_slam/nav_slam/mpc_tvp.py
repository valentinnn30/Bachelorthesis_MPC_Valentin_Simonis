...
from nav_slam.mpc_config_tvp import get_model, get_mpc, get_mpc_bcuonly
...
class MPCControlNode(Node):
    def __init__(self):
        ...
        self.bcu_vel_publisher = self.create_publisher(Float32, '/bcu_vel', 10)
        self.bcu_actual_publisher = self.create_publisher(Float32, '/bcu_actual', 10)

        self.error_publisher = self.create_publisher(Float32MultiArray, '/mpc_error', 10) # Optional: to publish MPC error for monitoring
        ...
        self.vy = 0.0
        self.vz = 0.0  # Vertical velocity, can be updated in depth_callback
        ...
        self.create_subscription(
            Float32, # Single float for depth
            '/depth_desired', # Topic to send just a new depth
            self.set_target_depth_callback,
            10
        )
        # yaw is not actively controlled, therefore no update in reference
        ...
        self.vel_bcu_log = []
        self.force_bcu_log = []
        self.reference_log = []  # List of dicts or arrays for each timestep
    ...
    def set_target_pose_callback(self, msg: Float64MultiArray):
        """
        Callback to update the full target pose (x, y, depth, yaw).
        Expected msg.data: [target_x, target_y, target_depth, target_yaw_radians]
        """
        if len(msg.data) == 2:
            self.reference_goal['x'] = msg.data[0]
            self.reference_goal['y'] = msg.data[1]
            self.get_logger().info(
                f"Updated Target Pose: X={self.reference_goal['x']:.2f}, "
                f"Y={self.reference_goal['y']:.2f}"
            )
        else:
            self.get_logger().warn(
                f"Invalid set_target_pose message data length: {len(msg.data)}. "
                "Expected 2 elements: [x, y]."
            )
            

    def set_target_depth_callback(self, msg: Float32):
        """
        Callback to update only the target depth.
        Expected msg.data: single float for target depth.
        """
        new_depth = msg.data
    ...
    def control_loop_callback(self):
        ...
        bcu_velo = Float32()
        bcu_velo.data = self.current_x0[6] # BCU input typically an integer
        self.bcu_vel_publisher.publish(bcu_velo)
        ...
        if self.time is not None:
            ...
            dt = (self.time - self.previous_time).nanoseconds * 1e-9
        else:
            self.time = self.get_clock().now()
            dt = 0.2
        ...
        self.bcu_actual_pos = np.clip(self.bcu_actual_pos, 50.0, 100.0)
        ...
        # --- Publish BCU Manual Input ---
        bcu_msg = Int32()
        bcu_msg.data = int(round(BCU_input.item())) # BCU input typically an integer
        ...
        self.reference_log.append([
            self.reference_goal['x'],
            self.reference_goal['y'],
            self.reference_goal['depth'],
            self.reference_goal['yaw'],
            self.reference_goal.get('bcu_pos', 75.0)  # Default if missing
        ])
    ...
    def plot_results(self):
        ...
        output_dir = '/home/reefranger/Desktop/plots'
        ...
        try:
            ...
            ref_log = np.array(self.reference_log)
        ...
        for i in range(min(len(u_force_log), len(yaw_angles_rad))):
        ...
        axs[2].grid(True)
        plt.tight_layout()
        fig.savefig(os.path.join(output_dir, 'Pos.png'))
        ...
        plt.legend()
        plt.grid(True)
        fig.savefig(os.path.join(output_dir, 'Thrust.png'))
        ...
        plt.plot(ref_log[:, 4], '--', label='Target BCU Pos', color='purple')
        plt.xlabel('Time Step')
        plt.ylabel('BCU Value')
        plt.title('MPC Commanded BCU Input vs. Actual BCU Position')
        plt.legend()
        plt.grid(True)
        fig.savefig(os.path.join(output_dir, 'BCU.png'))
        ...
        plt.legend()
        plt.grid(True)
        fig.savefig(os.path.join(output_dir, 'Bodyforce.png'))
        ...
        plt.grid(True)
        plt.tight_layout()
        fig.savefig(os.path.join(output_dir, 'Globalforce.png'))
        ...
        plt.legend()
        plt.grid(True)
        fig.savefig(os.path.join(output_dir, 'vertvel.png'))
        ...
        plt.legend()
        plt.grid(True)
        fig.savefig(os.path.join(output_dir, 'vertforce.png'))
